...
void update(float dt, Tank & tank, std::vector<Snowflake> & snowflakes, std::vector<Box>& boxes, Box & target_box,
    ...
    if (t < 0) {
        ...
        if (snowflakes.size() < 100) {
            ...
            new_snowflake.dir_angle = dis(gen);
    ...
    // проверяем колизии
    // удаляем упавшие снежинки попавшие под танк
    auto isEven_snowflakes = [&tank](Snowflake snowflake) { 
        return (snowflake.is_stuck && (!snowflake.is_stuck_on_chassi)) &&
                tank.isPointInsideTopOfСhassi(Point3D(snowflake.position.x, snowflake.position.y, 1.9 * tank.scale));
    };
    snowflakes.erase(std::remove_if(snowflakes.begin(), snowflakes.end(), isEven_snowflakes), snowflakes.end());
